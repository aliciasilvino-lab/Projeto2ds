...
let valorIR = salarioBruto * (percIR / 100);
...
let totalDescontos = valorIR + inss + sindicato + valeAlimentacao + valeTransporte;
let salarioLiquido = salarioBruto - totalDescontos;

console.log(`Salário Bruto: (${valorHora} * ${horas}) : R$ ${salarioBruto.toFixed(2)}`);
console.log(`(-) IR (${percIR}%) : R$ ${valorIR.toFixed(2)}`);
console.log(`(-) INSS (10%) : R$ ${inss.toFixed(2)}`);
console.log(`(-) SINDICATO (3%) : R$ ${sindicato.toFixed(2)}`);
console.log(`(-) V. ALIMENTAÇÃO (8%) : R$ ${valeAlimentacao.toFixed(2)}`);
console.log(`(-) V. TRANSPORTE (6%) : R$ ${valeTransporte.toFixed(2)}`);
